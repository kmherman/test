...
md`You can also pass in something that produces a promise:`
  ...
  main.variable(observer()).define(["table","json_url"], function(table,json_url){return(
table(
...
md`If you use \`viewof\`, you can also assign the data produced by the first argument in the same cell:`
  ...
  main.variable(observer("viewof data")).define("viewof data", ["table","json_url"], function(table,json_url){return(
table(
  fetch(json_url).then(d => d.json())
)
)});
...
table(
  ...
  main.variable(observer("table")).define("table", ["loader","defaultHumanize","html","parseQuery","DOM","CSV"], function(loader,defaultHumanize,html,parseQuery,DOM,CSV){return(
...
${
  ...
  main.variable(observer("parseQuery")).define("parseQuery", function(){return(
(q, headers) => {
  if (q == '') {
    return () => true;
  } else if (q.match(/[=<>]/)) {
  ...
  main.variable(observer("loader")).define("loader", ["html"], function(html){return(
html`<style>
			.loader,
            .loader:before,
            .loader:after {
  ...
  main.variable(observer("defaultHumanize")).define("defaultHumanize", function(){return(
str => str.replace(/[-_]/g, ' ')
)});
  main.variable(observer("json_url")).define("json_url", function(){return(
'https://gist.githubusercontent.com/gampleman/0903af7279c6c60e6d5c85ca33361960/raw/b21a86ceb808307208970b24045cfe9bc9a6f744/rainlevels.json'
  ...
  main.variable(observer("CSV")).define("CSV", ["require"], function(require){return(
